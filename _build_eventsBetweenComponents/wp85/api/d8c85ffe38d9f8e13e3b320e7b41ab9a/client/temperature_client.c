...
)
{
    ...
    LE_FATAL_IF(clientThreadPtr==NULL,
                "temperature_ConnectService() not called for current thread");
...
static void InitCommonData(void)
{
    ...
    _ClientDataPool = le_mem_CreatePool("temperature_ClientData", sizeof(_ClientData_t));

    // Allocate the client thread pool
    _ClientThreadDataPool = le_mem_CreatePool("temperature_ClientThreadData", sizeof(_ClientThreadData_t));
    ...
    _HandlerRefMap = le_ref_CreateMap("temperature_ClientHandlers", 5);
}
...
static void _Handle_temperature_AddIncomingMessageHandler
(
    void* _reportPtr,
    void* _dataPtr
)
{
    ...
    temperature_temperatureCbhFunc_t _handlerRef_temperature_AddIncomingMessageHandler = (temperature_temperatureCbhFunc_t)_clientDataPtr->handlerPtr;
    void* contextPtr = _clientDataPtr->contextPtr;

    // Unpack the remaining parameters
    char key[17];
    _msgBufPtr = UnpackString( _msgBufPtr, key, 17 );

    double value;
    _msgBufPtr = UnpackData( _msgBufPtr, &value, sizeof(double) );


    // Call the registered handler
    if ( _handlerRef_temperature_AddIncomingMessageHandler != NULL )
    {
        _handlerRef_temperature_AddIncomingMessageHandler( key, value, contextPtr );
    }
    else
    {
...
)
{
    ...
    _msgRef = le_msg_CreateMsg(GetCurrentSessionRef());
    _msgPtr = le_msg_GetPayloadPtr(_msgRef);
    _msgPtr->id = _MSGID_temperature_RemoveIncomingMessageHandler;
    _msgBufPtr = _msgPtr->buffer;
    ...
    _UNLOCK
    addHandlerRef = (temperature_IncomingMessageHandlerRef_t)clientDataPtr->handlerRef;
    le_mem_Release(clientDataPtr);
    _msgBufPtr = PackData( _msgBufPtr, &addHandlerRef, sizeof(temperature_IncomingMessageHandlerRef_t) );
...
)
{
    ...
    switch (msgPtr->id)
    {
        case _MSGID_temperature_AddIncomingMessageHandler :
            le_event_QueueFunctionToThread(callersThreadRef, _Handle_temperature_AddIncomingMessageHandler, msgRef, clientDataPtr);
            break;
